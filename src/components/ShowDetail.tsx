          ...
          <div className="relative z-10 p-6 sm:p-8 lg:p-10">
            <div className="flex flex-col sm:flex-row items-start sm:items-end gap-6">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm font-medium text-gray-300 mb-2 uppercase tracking-wider">
                  Concert
                </p>
                <button
                  onClick={() => {
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  <button
                    onClick={() => {
                      const shareUrl = `${origin}/shows/${
                        show?.slug
                      }?utm_source=setlistvote&utm_medium=share&utm_campaign=${encodeURIComponent(
                        show?.artist?.name || ""
                      )}-${encodeURIComponent(show?.slug || "")}`;
                      const shareText = `Vote on the predicted setlist for ${
                        show?.artist?.name || ""
                      }'s show at ${show?.venue?.name || ""} on ${showDate.toLocaleDateString(
                        "en-US",
                        {
                          month: "long",
                          day: "numeric",
                      ...
                      )}! Join the community at SetlistVote. ${shareUrl}`;
                  ...
                  {/* Second button to replace the dangling button */}
                  <button
                    onClick={() => {
