    ...
    <AnimatePresence mode="wait">
      {isSubscribed ? (
        <motion.button
          ref={ref}
          className={cn(
            "relative flex h-8 w-fit items-center justify-center overflow-hidden rounded-lg bg-primary px-3 text-primary-foreground text-xs",
            className,
          )}
        ...
        <motion.button
          ref={ref}
          className={cn(
            "relative flex h-8 w-fit cursor-pointer items-center justify-center rounded-lg border border-border bg-background px-3 text-foreground text-xs hover:bg-accent",
            className,
          )}
          onClick={(e) => {
            if (!isControlled) {
